...
### Implementation
- `src/enrichment-dedup.ts` (new): `EnrichmentDedup` (per-scope LRU, max 2000 scopes × 64 keys, injectable clock, 128-bit truncated SHA-256 content hashes) + `renderEnrichmentParts` (decision → envelope-wrapped parts + byte stats) + `invalidateScope` (forward-failure recovery — see below). State is in-memory only; plugin restart ⇒ full re-injection (the desired recovery semantics, #189 open question 1).
- Forward-failure consistency (round-1 + round-2 review): `filter()` records blocks as injected BEFORE the channel notification is sent, so a failed forward would leave dedup state claiming Claude saw content it never received — the next turn would suppress blocks with a stub pointing at a history copy that doesn't exist. Two layers: (a) the enqueue callback wraps enrich+forward in try/catch → `invalidateScope` + rethrow; (b) because the index.ts message handler intentionally swallows notification errors (pre-existing log-and-continue, kept so the doc-comment / reaction / flush-injection call sites are unaffected), the `MessageHandler` contract now allows resolving `false` to signal "delivery failed but handled" — the IM path treats that the same as a throw and invalidates the scope. Round-1's catch alone was dead code for the transport-error case (round-2 finding 1). Over-invalidation is safe: costs one re-injection, never correctness.
- Round-2 review hardening: (i) suppressed hits refresh the entry's insertion position (NOT its `at`) so the inner-cap backstop evicts idle keys instead of the always-suppressed profile entry; (ii) duplicate @-mentions of the same user in one message are deduped before block collection — previously the second copy rendered a same-turn "unchanged" stub; (iii) dedup scope key now uses the MessageQueue's exact shape (`chatId::threadId`, empty→`_`) so dedup partitioning can never disagree with queue serialization; (iv) `LARK_MEMORY_DEDUP_WINDOW_MS` is clamped to a 24 h maximum (the absolute-TTL re-injection is the only recovery from unobservable Claude-side compaction/clear, so the window must stay bounded) and a value resolving to ≤ 0 logs a stderr breadcrumb instead of disabling silently; (v) `RenderStats.stubBytes` reports the emitted stub cost so the `[enrich-dedup]` measurement line states net savings honestly.
...
- `scripts/enrichment-dedup-smoke.ts` (new, wired into `scripts/test.sh`): 18 cases — disable switch, suppress/re-inject on hash change, window expiry, absolute-TTL semantics (suppression must not refresh the timestamp), scope isolation (thread vs thread vs thread-less), group multi-user profile keying, outer-LRU eviction, decision ordering, stub rendering through the envelope (incl. exact `label="… · unchanged"` attribute and hostile-label escaping), silent omission for episode kinds, stub-byte accounting, `invalidateScope` recovery (incl. no-leak into other scopes + no-op on unknown scope), negative-window disable, inner-cap eviction preferring idle keys over suppressed-live ones, and exact window-boundary re-injection.
...
### Implementation
- `src/channel.ts:handleCommentEvent`: replaced single `fileComment.get` call with parallel `Promise.allSettled([fileCommentReply.list, fileComment.list])` (round 1 I-1 — `Promise.all` would have coupled body delivery to quote-list success, so a transient `fileComment.list` failure would have wiped the resolved body). Page size capped at 100 — if a comment has >100 replies or a doc has >100 comments, the matching item may not be in the first page; current behavior would show `<body unknown="true">` (existing add_reply fallback) for missing-reply or omit `quote` for missing-comment. Pagination support is a follow-up.
- `CommentEventDeps.client` interface: removed `fileComment.get`, added `fileComment.list` and `fileCommentReply.list` shapes.
- `scripts/comment-event-smoke.ts`: rewrote `makeDeps` default factory + all 5 ad-hoc per-case clients (cases 6, 7, 8, 12, 14) to mock the new endpoints. Added case 19 explicitly exercising the anchored-comment path (verifies both list calls fire + quote surfaces in `<selected_text>` + body in `<body>`). Added case 7c (round 1 M-3) to lock in the `allSettled` partial-failure semantics: quote-list rejection alone renders body and omits `<selected_text>` without surfacing `<fetch_error>`. Tightened case 6 (round 1 M-4) to assert `<selected_text>q</selected_text>` actually renders given the existing `quote: 'q'` mock — was previously testing parent/body only. All 20 prior cases preserve their semantic intent under the new shape.
- Endpoint behavior live-verified against open.feishu.cn 2026-06-07 per the #185 repro — `fileComment.list` returns `is_whole=false` items in its default response (no explicit `is_whole` param needed); `fileCommentReply.list` returns the original comment as `items[0]` (Feishu's data model treats the comment as the first reply in its own thread).
...
- **Feishu doc-comment @bot channel** (#181, #182). Closes the doc-comment collaboration loop: a user @-mentions the bot in a cloud-document comment → bot fetches the comment context → Claude reasons about it → bot replies in the same comment thread, attributed to the bot's app identity (`tenant_access_token`). Tenant-wide coverage with **zero per-file subscribe** — bot just needs the app scopes and event subscription enabled in the Feishu dev console; no `subscribe_doc` tool, no per-document state.
  ...
  The plugin subscribes to `drive.notice.comment_add_v1` and filters events down to those where `is_mentioned=true` AND `from_user_id != bot` (loop prevention). When a qualifying event arrives, `handleCommentEvent` pre-fetches the comment body, the parent (for replies), and the doc title; assembles a `<doc_comment ...>` envelope; binds the event-time caller per-comment via `identitySession.setCaller("doc:<file_token>", comment_id, from_user_id.open_id)`; and routes via a synthetic `chat_id = "doc:<file_token>"` (with `thread_id = comment_id` carried in notification meta so the per-comment binding is reversible) so sensitive tools resolve to the actual triggering user. Two new MCP tools — `reply_doc_comment` and `create_doc_comment` — wrap the Feishu reply/create-comment APIs with an explicit owner-only gate (`auth.caller === identitySession.getOwner()`) and a `doc_token` binding check (`chat_id` suffix must match `args.doc_token`) so a prompt-injected model cannot post into a different document than the triggering one. The Stop hook recognizes the new envelope kind and only accepts `reply_doc_comment` with matching `doc_token` + `comment_id` as a satisfier (or `[LARK_DEFER]` / `[LARK_NO_REPLY]` sentinels as before).
...
- `src/channel.ts`: new top-level `handleCommentEvent(data, deps)` pure function with `CommentEventDeps` dependency interface (testable without instantiating the SDK); third `.register({ 'drive.notice.comment_add_v1': ... })` chained on the existing `EventDispatcher`; private `commentEventIdSeen: TTLCache<string, true>` holds dedup state with a 60-minute TTL. New helpers `extractText`, `escapeAttr`, `escapeBody`, `buildDocCommentEnvelope` (escape `&` first to avoid double-encoding `&quot;` / `&lt;` / `&gt;`). Smoke at `scripts/comment-event-smoke.ts` (20 cases).
- `src/identity-session.ts`: exports `DOC_CHAT_ID_PREFIX = 'doc:'` as a naming convention used by `handleCommentEvent` to construct the synthetic chat_id. The session map is populated at event time via `setCaller("doc:<file_token>", comment_id, from_user_id.open_id)` (per-comment keying to avoid cross-event session-overwrite races) and consulted through the normal `getCaller` path — there is NO prefix shortcut to owner. Tools that require owner identity (`reply_doc_comment`, `create_doc_comment`) compare the resolved caller against `getOwner()` explicitly. Identity-smoke now at 16/16 (covers per-comment keying + LRU eviction + doc:-invariant throw + maxSize=0 clamp) including a security-regression test ensuring `setCaller("doc:<token>", comment_id, "ou_alice")` resolves to Alice, not owner.
...
- `src/identity-session.ts`: removed an unsafe `chatId.startsWith(DOC_CHAT_ID_PREFIX)` short-circuit in `getCaller` that returned `ownerFallback()` regardless of the session-map entry. Because `getCaller` is the global identity resolver used by ALL sensitive tools (`save_memory`, `create_job`, `delete_job`, ...), the shortcut let any non-owner who could @-mention the bot in a doc comment escalate to owner identity through any prompt-injected tool call. Found by PR #182 review (P1, commit `4f59dce`).
- `src/channel.ts`: `handleCommentEvent` normalized `c.reply_list?.items` first; the SDK actually returns `c.reply_list?.replies`. In production every add_comment surfaced as `<body unknown="true">` and every add_reply hit a TypeError. Smoke tests masked the bug by mocking `items`. Found by PR #182 review (P2 first-round, commit `62d940c`).
- `src/tools.ts`: `reply_doc_comment` / `create_doc_comment` accepted `args.doc_token` without binding it to the triggering doc. A prompt-injected model could post into any doc the bot collaborates on. Now requires `doc_token === chat_id.slice("doc:".length)` when chat_id is `doc:<token>`. Found by PR #182 review (P2 second-round, this commit).
- `hooks/enforce-lark-reply.mjs`: a matching `reply_doc_comment` tool_use was treated as satisfying the doc_comment obligation regardless of whether the call succeeded. Multiple `isError` return paths (non-owner gate, empty/oversize content, doc_token mismatch, Feishu API errors, permission_denied/1069302) silently exited the hook cleanly while no comment was posted — under the common case of a non-owner @-mention reaching the owner-only tool, the deny was the COMMON outcome, leaving the user with silence and Claude no chance to remediate. Hook now builds a `tool_use_id → tool_result` index over the turn entries and only counts `reply_doc_comment` as a satisfier when (a) a `tool_result` block exists AND (b) `is_error !== true`. Errored or missing-result tool_uses leave the doc_comment pending — Claude must remediate (e.g. via `[LARK_DEFER]` sentinel acknowledging the deny). IM `reply` / `react` satisfiers stay tool_result-agnostic (separate operational model: auth established at inbound time; tool-level defer via `tool_result [LARK_DEFER]` injection at #122 is the existing remediation path). Audit log gains a `doc_comment_obligation_unsatisfied` line per skipped attempt with `tool_use_id` and `reason=tool_error|missing_result`. Test harness gains 3 cases (now 113): tests 61 (errored result → no satisfy), 62 (missing result → no satisfy), 63 (errored result + `[LARK_DEFER]` → satisfies); existing test 55 updated to include a SUCCESS tool_result. Found by PR #182 review (P2 third-round, this commit).
- `src/channel.ts`: `handleCommentEvent` skipped `passesWhitelist`. Operators relying on `LARK_ALLOWED_USER_IDS` had their gate silently bypassed for doc-comment events — any tenant user with comment access to any doc the bot had been @-mentioned in (auto-collaborator) could reach Claude through this channel. Whitelist now applies. Found by PR #182 review (round 4 C1).
- `src/channel.ts`: doc-comment session entries were keyed only at chat level (`setCaller("doc:<token>", undefined, ...)`). Concurrent events on the same doc overwrote each other — DoS-by-race against owner's in-flight reply. Now keyed per comment_id; `reply_doc_comment` / `create_doc_comment` accept `thread_id` (comment_id) from notification meta to resolve identity. Found by PR #182 review (round 4 I1).
- `src/tools.ts`: `reply_doc_comment` / `create_doc_comment` accepted `chat_id="__terminal__"` and skipped the doc_token binding check, allowing bot to post on any doc the model named. Combined with the whitelist bypass, this was a real escalation surface. The escape hatch is now closed — `chat_id` MUST start with `doc:`. Found by PR #182 review (round 4 I2).
- `CLAUDE.md`: "Doc-comment events" key-design bullet did not reflect the post-review security model (no mention of `passesWhitelist`, per-comment session keying, doc_token binding, or `tool_result` requirement). Same drift class as the CHANGELOG fix in `fead4cd`. Now updated to track the live code. Found by PR #182 review (round 4 I3).
- `src/channel.ts`: doc-comment events were dropped silently when an operator configured only `LARK_ALLOWED_CHAT_IDS` (no user list). The synthetic chat_id `doc:<file_token>` could never match a real chat-list entry, so `passesWhitelist` returned false for every event. Replaced with a dedicated `passesDocCommentWhitelist` that gates on `LARK_ALLOWED_USER_IDS` when configured and otherwise passes (Feishu-side ACL — bot must be doc collaborator + @-mentioned — is the upstream boundary). Found by PR #182 review (round 5 I-1).
- `src/identity-session.ts`: per-comment session entries (post-I1) grow 1-per-comment, uncapped beyond the 2h TTL. A busy bot could accumulate tens of thousands of entries under hostile commenting. Added LRU cap (default 5000, env-tunable via `LARK_IDENTITY_SESSION_MAX_SIZE`); inserts past capacity evict the oldest entry. Found by PR #182 review (round 5 N-1).
- `src/channel.ts`: `capUtf8` truncated mid-codepoint, emitting U+FFFD at the boundary. Now snaps cut back to the last valid UTF-8 leading byte. Found by PR #182 review (round 5 N-2).
- `src/identity-session.ts`: the post-I1 invariant ("doc: chat_ids are ONLY ever keyed per-comment") was implicit. `setCaller` now throws when called with a `doc:` chat_id and undefined thread_id, locking the contract at runtime. Found by PR #182 review (round 5 N-5).
- `src/tools.ts` / `src/identity-session.ts` / `src/prompts.ts`: in-source descriptions and JSDoc still narrated the pre-round-4 model (chat_id description advertising `__terminal__`, JSDoc showing `setCaller(..., undefined, ...)`, `mcpServerInstructions` missing the doc-comment tools). All three swept in one pass. Found by PR #182 review (round 5 I-2/I-3/I-4).
- `src/prompts.ts`: `mcpServerInstructions` ambiguous about when `__terminal__` is a legitimate pass-through value (terminal turns from `/lark:jobs`) vs an illegitimate substitution. Reworded to make the carve-out explicit. Found by PR #182 review (round 6 M-1).
- `src/identity-session.ts`: LRU max-size was hardcoded. Added `LARK_IDENTITY_SESSION_MAX_SIZE` env override matching other tracker-cap conventions; `maxSize` is clamped to a floor of 1 to keep the cap effective at the corner. Found by PR #182 review (round 6 M-2/M-3).
- `README.md` / `README_CN.md`: identity/privacy env-var tables missed `LARK_IDENTITY_SESSION_MAX_SIZE` (added to `src/config.ts` in `b2f5d80` but never propagated to operator-facing docs). Found by PR #182 review (round 7 M-1).
- `src/identity-session.ts`: `Math.max(1, NaN) === NaN`, so a direct caller passing `maxSize: NaN` would silently disable the LRU cap. Not reachable through the env path (`optionalPositiveNumber` rejects non-finite), but added `Number.isFinite`-guarded fallback for defense-in-depth. Found by PR #182 review (round 7 M-2).
- `README.md` / `README_CN.md`: env-table row for `LARK_IDENTITY_SESSION_MAX_SIZE` claimed "Range [1, 1_000_000]" but the code has no upper bound — fictional constraint. Rewrote to "minimum 1; no upper bound — operators choose based on RAM budget". Found by PR #182 review (round 8 I-1).
- `scripts/identity-smoke.ts`: case 17 inserted 10 entries and asserted `_size() === 10`, which passes both pre-fix (NaN cap silently disabled → 10 land) and post-fix (cap=5000 → 10 land) — didn't actually distinguish the fix. Added `_maxSize()` test accessor and rewrote case 17 + new case 17a to assert `Number.isFinite(_maxSize()) && _maxSize() === 5000`. Found by PR #182 review (round 8 I-2).
- `CHANGELOG.md`: round-7 M-1 / M-2 fix entries were not added to this section in `49551df` (the very commit that swept docs-lag-code). Added retroactively. Found by PR #182 review (round 8 I-3).
